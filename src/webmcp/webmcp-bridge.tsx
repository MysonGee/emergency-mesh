...
import { compareSupportScenario, evaluateMainDemoWithAssets, findMinimumInterventions, findNeighbourSupport, findSinglePointsOfFailure, simulateResourceUnavailable } from "@/domain/network";
...
export function WebMcpBridge({ showStatus = true, sourceAssets = assets }: { showStatus?: boolean; sourceAssets?: Asset[] }) {
  ...
  useEffect(() => { sourceAssetsRef.current = sourceAssets; }, [sourceAssets]);
  useEffect(() => {
    ...
    const register = async (context: NonNullable<Document["modelContext"]>) => {
      ...
      await context.registerTool({ name: "get_unit_readiness", description: "Return deterministic fictional Harbour Station readiness, including browser-local demo overrides.", inputSchema: emptySchema, execute: async () => report("Agent requested station readiness", evaluateMainDemoWithAssets(sourceAssetsRef.current)) });
      await context.registerTool({ name: "get_people_and_asset_evidence", description: "Return declared member and asset evidence for the fictional Harbour station, including browser-local demo overrides. This does not create a roster, alter an upstream source, or create operational work.", inputSchema: emptySchema, execute: async () => report("Agent requested people and asset evidence", { members: members.filter((member) => member.unitId === "harbour").map(({ id, name, sourceStatus, unitId }) => ({ id, name, sourceStatus, unitId })), assets: sourceAssetsRef.current.filter((asset) => asset.unitId === "harbour").map(({ id, name, type, status, offlineUntil, offlineSince, offlineReason }) => ({ id, name, type, status, offlineUntil, offlineSince, offlineReason })) }) });
      ...
      await context.registerTool({ name: "evaluate_capability_plan", description: "Evaluate the canonical simultaneous capability plan using deterministic constraints and browser-local demo overrides.", inputSchema: emptySchema, execute: async () => report("Agent evaluated the canonical plan", evaluateMainDemoWithAssets(sourceAssetsRef.current)) });
      await context.registerTool({ name: "simulate_resource_unavailable", description: "Run a scenario-only deterministic calculation after a named fictional member or asset becomes unavailable. It never changes a source system, activates volunteers, or creates operational work.", inputSchema: { type: "object", properties: { resource_id: { type: "string", description: "Fictional member or asset id, for example H07 or H-C01." } }, required: ["resource_id"] }, execute: async (input) => report(`Agent simulated ${String(input.resource_id)} unavailable`, simulateResourceUnavailable(String(input.resource_id), sourceAssetsRef.current)) });
      ...
      await context.registerTool({ name: "compare_support_scenario", description: "Compare the current fictional result with a time-bounded shared-support assumption. This is analysis only and never transfers or allocates a resource.", inputSchema: emptySchema, execute: async () => report("Agent compared the shared-support scenario", compareSupportScenario(sourceAssetsRef.current)) });
      await context.registerTool({ name: "find_minimum_interventions", description: "Find ranked, human-approved interventions for the canonical plan.", inputSchema: emptySchema, execute: async () => report("Agent searched minimum interventions", findMinimumInterventions(sourceAssetsRef.current)) });
      await context.registerTool({ name: "find_neighbour_support", description: "Find shareable local support; excludes offline options and anything that would breach supplying-station readiness.", inputSchema: emptySchema, execute: async () => report("Agent searched local support", findNeighbourSupport(sourceAssetsRef.current)) });
